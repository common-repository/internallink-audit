...
class Ila_Shared
{
    ...
    private function __construct()
    {
        ...
        load_plugin_textdomain('ila', false, 'internallink-audit/lang/');
        ...
        $lang_data=array();
         $ignore_words = $this->getAllIgnoreWordLists();
         
        foreach($ignore_words as $words){
            $lang_data[] = $words;
        }
        $this->data['ignore']=implode("|", $lang_data);
       //print_r($lang_data);
    ...
    public static function get_instance()
    {
     ...
     public static function getAllIgnoreWordLists(){
            ...
            $all_ignore_lists       = array();
                ...
                $ignore_words_file =  str_replace('shared','',plugin_dir_path( __FILE__ )). 'admin/inc/ignore_words_list.txt';

              // print_r($ignore_words_file);
    ...
    public function get_manual_interlinks($string)
    {
       ...
       // $string=str_replace(array('<em>', '</em>', '<strong>', '</strong>'), '', $string);
        ...
        $num_matches = preg_match_all(
            ...
            ([\'"]?)                #4 Match double quotes, single quote or no quote ( captured for the backreference \1 )
            ' . $site_url . '       #5 The site URL ( Scheme and Domain )
            ([\'"]?)                #6 Match double quotes, single quote or no
            >                       #7 End of the start-tag
            .*?                     #8 Link text or nested tags. After the dot ( enclose in parenthesis ) negative lookbehinds can be applied to avoid specific stuff inside the link text or nested tags. Example with single negative lookbehind (.(?<!word1))*? Example with multiple negative lookbehind (.(?<!word1)(?<!word2)(?<!word3))*?
            <\/a\s*>                #9 Element a end-tag with optional white-spaces characters before the >
            }ix',
            $string, $matches);
        $arr = [];
         $mm="(#|".$this->data['ignore'].")";
    ...
    public function get_manual_interlinks_bk($string)
    {
    ...
    public function add_autolinks($content, $check_query = true, $post_type = '', $post_id = false)
    {
         ...
         */
        $enable_ail = get_post_meta($post_id, '_ila_enable_ail', true);
        ...
        foreach ($autolinks as $key => $autolink) {
            ...
            if (intval(get_option($this->get('slug') . '_ignore_self_ail'), 10) == 1) {
                ...
                if ($autolink['url'] == substr($post_permalink, $home_url_length)) {
                    continue;
                }
            }
    ...
    public function calculate_link_juice($post_content_with_autolinks, $post_id, $link_position)
    {
        ...
        $seo_power = get_post_meta($post_id, '_ila_seo_power', true);
    ...
    /*
     * With versions lower than 1.19 the list of protected block is stored in the "ila_protected_tags" option as a
     * comma separated list of tags and not as a serialized array.
     *
     * This method:
     *
     * 1 - Retrieves the "ila_protected_tags" option value
    ...
    public function get_protected_tags_option()
    {
        ...
        $protected_tags_a = [];
        $protected_tags = get_option("ila_protected_tags");
    ...
    private function get_max_number_autolinks_per_post($post_id, $post_content)
    {
         ...
         */
        if (intval(get_option("ila_general_limit_subtract_mil"), 10) === 1) {
    ...
    public function generate_interlinks_optimization_metabox_html($post)
    {
       ...
       //     $html .= '<p>' . esc_attr__('The number of interlinks included in this post is optimized.', 'ila') . '</p>';
       // } else {
       //     $html .= '<p>' . esc_attr__('Please optimize the number of interlinks, this post currently has', 'ila') . '&nbsp' . $total_number_of_interlinks . '&nbsp' . _n('interlink', 'interlinks', $total_number_of_interlinks, 'ila') . '. (' . $number_of_manual_interlinks . '&nbsp' . _n('manual interlink', 'manual interlinks', $number_of_manual_interlinks, 'ila') . '&nbsp' . esc_attr__('and', 'ila') . '&nbsp' . $number_of_autolinks . '&nbsp' . _n('auto interlink', 'auto interlinks', $number_of_autolinks, 'ila') . ')</p>';
       //     if ($suggested_min_number_of_interlinks === $suggested_max_number_of_interlinks) {
       //         $html .= '<p>' . esc_attr__('Based on the content length and on your options their number should be', 'ila') . '&nbsp' . $suggested_min_number_of_interlinks . '.</p>';
       //     } else {
       //         $html .= '<p>' . esc_attr__('Based on the content length and on your options their number should be included between', 'ila') . '&nbsp' . $suggested_min_number_of_interlinks . '&nbsp' . esc_attr__('and', 'ila') . '&nbsp' . $suggested_max_number_of_interlinks . '.</p>';
       //     }
       // }
